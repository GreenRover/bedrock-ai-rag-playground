 ...
 */
@Slf4j
public class LoggingReRankingContentAggregator implements ContentAggregator {
    ...
    public LoggingReRankingContentAggregator(ScoringModel scoringModel,
    ...
    public static LoggingReRankingContentAggregatorBuilder builder() {
        return new LoggingReRankingContentAggregatorBuilder();
    }
    ...
    protected List<Content> reRankAndFilter(List<Content> contents, Query query) {
        ...
        log.debug("--- Reranking Results for Query: '{}' ---", query.text());

        var i = 0;
        List<Map.Entry<TextSegment, Double>> sortedEntries = new ArrayList<>(segmentToScore.entrySet());
        sortedEntries.sort(Map.Entry.<TextSegment, Double>comparingByValue().reversed());
        for (var entry : sortedEntries) {
            boolean isUsed = minScore == null || entry.getValue() >= minScore;

            String url = entry.getKey().metadata().getString("url");
            if (url == null) {
                url =  entry.getKey().metadata().getString("title_path");
            }

            if (i++ > maxResults) {
                log.debug("{} (OVER LIMIT) = {}", String.format("%.5f", entry.getValue()), url);
                continue;
            }

            // Print exactly what you requested
            log.debug("{} ({}) = {}", String.format("%.5f", entry.getValue()), isUsed ? "USED" : "FILTERED", url);

            if (isUsed) {
                finalResults.add(Content.from(entry.getKey(), Map.of(RERANKED_SCORE, entry.getValue())));
            }
        }
    ...
    public static class LoggingReRankingContentAggregatorBuilder {
        private ScoringModel scoringModel;
        private Function<Map<Query, Collection<List<Content>>>, Query> querySelector;
        private Double minScore;
        private Integer maxResults;

        LoggingReRankingContentAggregatorBuilder() {
        }

        public LoggingReRankingContentAggregatorBuilder scoringModel(ScoringModel scoringModel) {
            this.scoringModel = scoringModel;
            return this;
        }

        public LoggingReRankingContentAggregatorBuilder querySelector(Function<Map<Query, Collection<List<Content>>>, Query> querySelector) {
            this.querySelector = querySelector;
            return this;
        }

        public LoggingReRankingContentAggregatorBuilder minScore(Double minScore) {
            this.minScore = minScore;
            return this;
        }

        public LoggingReRankingContentAggregatorBuilder maxResults(Integer maxResults) {
            this.maxResults = maxResults;
            return this;
        }

        public LoggingReRankingContentAggregator build() {
            return new LoggingReRankingContentAggregator(this.scoringModel, this.querySelector, this.minScore, this.maxResults);
        }
    }
}
